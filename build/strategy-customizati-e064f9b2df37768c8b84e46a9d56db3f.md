...
关于如何设置 `dry` 或 `live` 模式，请查阅[配置文档](configuration.md#using-dry-run-mode)。
...
(why-cant-i-see-real-time-candle-data)=
...
完整示例见[完整数据提供者示例](#complete-dataprovider-sample)。
...
(get_pair_dataframepair-timeframe)=
...
(get_analyzed_dataframepair-timeframe)=
...
:::{note}
锁定会自动延长到下一个 K 线结束。如 5m 周期，锁到 10:18 会延长到 10:20。
:::
