...
你可以通过在 Hyperopt 子类中实现 `generate_estimator()`，为 Hyperopt 定义自己的 [optuna](https://optuna.org/) 采样器。
...
详情见 [optuna-samplers 文档](https://optuna.readthedocs.io/en/stable/reference/samplers/index.html)，也可以是继承自 `optuna.samplers.BaseSampler` 的类实例。

有时需要自行研究以发现更多采样器（如 [optunahub](https://hub.optuna.org/) 提供的）。

:::{note} 默认值 "NSGAIIISampler"
