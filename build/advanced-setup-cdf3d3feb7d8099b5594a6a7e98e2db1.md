...
用于指定这些文件路径的可选参数是 `--db-url`，它需要一个有效的 `SQLAlchemy url`，你可以参考这个[链接](SQLAlchemy_URL_参考手册.md)。
...
freqtrade trade -c MyConfigUSDT.json -s MyCustomStrategy 
    --db-url sqlite:///user_data/tradesUSDT.dryrun.sqlite
```
...
# 终端 1：
freqtrade trade -c MyConfigBTC.json -s MyCustomStrategy 
    --db-url sqlite:///user_data/tradesBTC.live.sqlite
# 终端 2：
freqtrade trade -c MyConfigUSDT.json -s MyCustomStrategy 
    --db-url sqlite:///user_data/tradesUSDT.live.sqlite
```
...
要用 docker 运行多个 freqtrade 实例，你需要编辑 `docker-compose.yml` 文件，将所有实例作为独立服务添加。

你可以将配置拆分为多个文件，便于模块化管理，这样如果需要修改所有机器人的通用配置，只需改一个文件即可。
...
[PostgreSQL Docker Step By Step](https://www.datacamp.com/tutorial/postgresql-docker)

[MariaDB Docker 安装方法](https://mariadb.com/docs/server/server-management/install-and-upgrade-mariadb/installing-mariadb/binary-packages/automated-mariadb-deployment-and-administration/docker-and-mariadb/installing-and-using-mariadb-via-docker)
:::
...
你也可以将默认输出流配置为 JSON 格式。"fmt_dict" 属性定义了 json 输出的键及 [python logging LogRecord 属性](https://docs.python.org/3/library/logging.html#logrecord-attributes)。
